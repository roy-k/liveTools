...
<div class="vue-upload-file" v-show="value">
	<div class="vuf-drop-area"
		...
		<input type="file" v-show="false" @change="handleChange">
	</div>
...
export default {
	...
	methods: {
		...
		upload(files){
			let that = this,
				{url, params, onlySingle, field, copsIndex, lang} = this,
				fmData = new FormData();
			...
			new Promise(function (resolve, reject) {
				...
				function (resData) {
					that.loading = 2;
					that.$dispatch('uploadSuccess', resData, field, copsIndex);
				},
				// 上传失败
				function (sts) {
					that.loading = 3;
					...
					that.$dispatch('uploadFail', sts, field, copsIndex);
				}
			);
		}
	}
